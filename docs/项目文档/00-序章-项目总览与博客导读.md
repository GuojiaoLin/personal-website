...
summary: 这是一套「把个人网站当成正经工程做」的技术博客。不是讲怎么搭一个静态主页，而是复盘这个 React + Spring Boot 全栈项目里，我在架构选型、自研 Markdown 引擎、会话认证、内容建模、评论系统、图册与文件上传安全上做了哪些真实取舍。每一章对应一个面试官真的会考的能力。
tags: [全栈, 架构, 面试]
category: 总览
order: 0
---
...
- 前端：React 18 + Vite + TypeScript 的 SPA，覆盖首页、技术博客、摄影作品、关于我和后台管理，带一个**完全自研的 Markdown 博客引擎**；
- 后端：Spring Boot 3.3.7 + Spring Security + JPA + Flyway + PostgreSQL 的 REST API，带一套**自建的内容管理后台（CMS）**，现在已经能管理项目、博客、评论、媒体、图册、简历、About 内容和首页图片槽位；
...
flowchart TD
  ...
  subgraph Vite["Vite Dev / 构建"]
    ...
    IMG["构建期图片优化<br/>sharp + vite-plugin-image-optimizer"]
  end

  subgraph Spring["Spring Boot 后端"]
    SEC["Spring Security<br/>会话认证 + BCrypt + 角色鉴权"]
    CTRL["Controller 层<br/>Project / BlogPost / Comment / Media / Gallery / Home / About / Resume"]
    SVC["Service 层<br/>事务 + 业务校验 + 图片优化/派生图"]
    REPO["Repository 层<br/>Spring Data JPA"]
    STATIC["静态资源处理器<br/>/uploads/** 映射本地目录"]
  end

  DB[("PostgreSQL<br/>Flyway 管理迁移")]
  FS[["本地 uploads 目录<br/>博客图 / 项目图 / About 图 / 图册派生图 / 首页槽位图"]]
...
| 图像处理 | 后端 ImageIO / EXIF 方向校正 / 图册 thumbnail+medium 派生图，前端构建期 sharp + vite-plugin-image-optimizer |
...
5. **上传文件仍然落本地磁盘**：现在通过压缩、目录校验和静态资源映射保证单机可用，但多实例生产环境更适合迁到对象存储 + CDN，[第 6 章](06-文件上传与静态资源安全.md)会讲这个边界。
