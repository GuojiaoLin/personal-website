...
summary: 文件上传是后端最容易踩坑的地方之一。这篇拆解项目里图片上传的完整链路：类型和大小校验、文件名清洗、目录穿越防护、重名自增、上传和物理文件的一致性、图片压缩与派生图生成，以及怎么把本地目录安全地对外当静态资源服务。每一步都对应一个真实的攻击面。
...
后台写博客要插图，项目卡片、About 页面、摄影图册和首页图片槽位也都要上传图片。听起来简单——接收文件、存盘、返回 URL。但「文件上传」是后端公认的高危区，随手一写就能踩中：
...
这一章先用 [MediaAssetService](../../backend/src/main/java/com/guojiaolin/website/media/MediaAssetService.java) 讲清上传安全的基础链路，再补上现在代码里通用的 [ImageUploadOptimizationService](../../backend/src/main/java/com/guojiaolin/website/common/ImageUploadOptimizationService.java) 和图册专用的 [GalleryImageDerivativeService](../../backend/src/main/java/com/guojiaolin/website/gallery/GalleryImageDerivativeService.java)。
...
这张图以博客配图为例。项目图标、About 图片、首页槽位图片和图册照片走的入口不同，但底层都绕不开同一组问题：权限、大小、类型、文件名、目录边界、重名、落盘 URL 和数据库记录。
...
## 6. 图片优化：大图压缩、EXIF 方向和图册派生图

上传安全解决的是「能不能安全地存」，图片优化解决的是「存下来之后会不会太大、方向会不会错、列表页会不会加载原图」。当前代码里分两层做。

第一层是通用上传优化：[ImageUploadOptimizationService](../../backend/src/main/java/com/guojiaolin/website/common/ImageUploadOptimizationService.java)。它主要处理 JPEG / PNG：

- 小图直接保留，避免为了压缩而损伤质量；
- 超过 500KB 或最长边超过 1600px 的图，会按最长边限制到 1600px 左右；
- JPEG 压缩质量约 0.78，PNG 大图也会转成 JPEG，减少体积；
- 读取 EXIF 方向信息，先把图片转正再保存，避免手机竖拍图上传后横着显示。

第二层是图册专用派生图：[GalleryImageDerivativeService](../../backend/src/main/java/com/guojiaolin/website/gallery/GalleryImageDerivativeService.java)。图册照片除了原图 URL，还会生成：

- `thumbnailUrl`：最长边约 640px，用在列表缩略图；
- `mediumUrl`：最长边约 1600px，用在详情或大图预览；
- 如果派生图生成失败，会回退到原图 URL，保证接口仍然可用。

所以 `GalleryPhoto` 现在有 `url / thumbnailUrl / mediumUrl` 三个地址，这对应 [V13 迁移](../../backend/src/main/resources/db/migration/V13__add_gallery_photo_derivatives.sql)。这不是为了炫技，而是为了让列表页别拿原图硬撑，用户看到的图片尺寸要和场景匹配。

## 7. 一致性与归属：文件、记录、博客三者对齐
...
| 大图拖慢页面 | 上传时压缩、限制最长边、处理 EXIF 方向 | ImageUploadOptimizationService |
| 图册列表加载原图 | 生成 thumbnail / medium 派生图 | GalleryImageDerivativeService / V13 |
| 多入口上传混乱 | 按业务目录和实体归属隔离 | media / gallery / home / about / projects |

## 10. 面试口述版

> 项目里的图片上传我是按「每一步对应一个攻击面」来设计的。入口先用授权收窄——上传接口在 `/api/admin/**` 下，匿名调不到。然后是大小和 MIME 白名单校验，框架层和业务层各一道。
>
> 最关键的是目录穿越防护：文件名先 `cleanPath` 并拒绝含 `..` 的，再用 `Path.getFileName()` 只取最后一段把目录部分剥掉，最后落盘前对 `resolve().normalize()` 的结果断言它仍 `startsWith` 上传目录——只要最终落点跑出目录就拒绝，删除操作也用同一套校验。重名会自增序号不覆盖。落库时把图片绑定到具体博客或图册/首页槽位等业务实体，后台能按归属隔离列举。
>
> 现在上传后还会做图片优化：大 JPEG/PNG 会限制最长边、压缩并处理 EXIF 方向；图册照片会额外生成 thumbnail 和 medium 两种派生图，列表页不直接吃原图。对外用 Spring 的资源处理器把本地目录映射成 `/uploads/**`。我也清楚两个局限：MIME 校验信的是客户端声明的类型、可伪造，严格场景要验 magic number；写盘和落库不在一个事务里，有孤儿文件的窗口。当前是站主自用后台、入口已被登录保护，所以接受这些简化。
...
**Q7：图册为什么要存 thumbnail 和 medium，不直接用原图？**
因为使用场景不同。列表页只需要小缩略图，如果直接加载原图，会浪费带宽也拖慢首屏；详情或预览需要更清晰的图，但也没必要永远用原始大图。所以图册上传后生成 thumbnail 和 medium 两档，前端按场景取合适的 URL。生成失败时回退到原图，保证功能不断。
